...
# covid data
covid <- read_csv(paste0(my_path,'raw/covid_10_13_2020_raw.csv'))
# population data
...
covid <- covid %>% select( -c( FIPS,Admin2,Last_Update,Lat,Long_,Combined_Key,Incidence_Rate,Case.Fatality_Ratio))

# One observation to be one country
# Check e.g. China:
covid %>% filter( Country_Region == 'China')
...
# Rename variables
covid2 <- covid2 %>% rename( country   = Country_Region ,
...
# 1) auxiliary table for countries without any population value
...
my_path <- "/Users/xinqi/Desktop/Data Analysis 2/COVID Assignment/data/"
# COVID data
write_csv( df , paste0(my_path,'clean/covid_pop_10_13_2020_clean.csv'))
